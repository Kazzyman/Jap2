...
var we_have_just_met bool
var game_or_no_game string

func beginGame() {
	if we_have_just_met {
		fmt.Println("Would you like to play a game?")
		_, _ = fmt.Scan(&game_or_no_game)
		if game_or_no_game == "yes" || game_or_no_game == "y" { // else we do not play a game
			we_have_just_met = false
			beginGame()
		} // else we do not play a game
	} else { // the user has asked to play a game
		we_have_just_met = false // remember that we have already met
		if nameOfPlayer == "" {  // only ask the user for his name if it is unknown
